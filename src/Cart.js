...
class Cart extends React.Component {
	...
	render() {
		...
		return (
			<React.Fragment>
				<div>
					<button className={`cart-btn ${Product.isOpen ? "drag_left" : "drag_right"}`} key={uuid()} onClick={() => { this.props.toggleCart(Product.isOpen); this.setState({ className: !this.state.className }) }}>
						{
							Product.isOpen ?
					...
					<div className={`cart ${Product.isOpen ? "open-cart" : "close"}`}>
						...
						<div className="subtotal-sec">
							...
							<button className={`checkout-btn ${this.state.className}`} onClick={() => {
								alert(total ? `$ ${total.toFixed(2)}` : "Add some items in cart")
							}}>CHECKOUT</button>
						</div>
					</div>
				</div>
